...
function exibirRow_geral() {
    ...
    for (x in arrayFilmes) {
        text += `<a href='especifico.html' onclick='definirEspecifico(${x})' title='${arrayFilmes[x].titulo}'><img src="${arrayFilmes[x].imagemCapa}" id="img-id-${x}"></img></a>`
    }
...
function exibirFilmesPorTitulos(tituloInput) {
    ...
    for (let x in arrayFilmes) {
        ...
        if (tituloFilme.toUpperCase().includes(tituloInput.toUpperCase())) {
            text += `<li><a href='especifico.html' onclick='definirEspecifico(${x})' target='_top'>${arrayFilmes[x].titulo}</a></li>`
        }
    }
    text += "</ul>"
